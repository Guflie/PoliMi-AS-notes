...
In the linear regression we have a linear relationship between the target and the predictors, what if the relationship isn't linear? Let's assume our target variable is a \textbf{binary variable}
$$
 Y_i=\begin{cases}
 1 & \text{student $i$ passes} \\
 0 & \text{student $i$ fails}
\end{cases}\ 
\Rightarrow
\prob (Y=1|X=x) = p
$$

If we try to use a linear regression model, we end up with something that ignores the actual variable range, spreading across the whole real numbers. 
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\linewidth]{img/logisticreg/linreg_exced_range.png}
\end{figure}

The idea to solve the range issue is to change the undeling shape: we shift from the world of probabilities to the world of \textbf{odds} := how more likely is some event to happen with respect to it not happen.
$$
 odds = \frac{p}{1-p}
$$
\begin{figure}[h]
...
and we then interpret them as follows:
\begin{itemize}
    \item $odds=1$ success and failure are equally possible
    \item $odds>1$ succcess is more probable than failure 
    \item $odds<1$ failure is more probable than success
\end{itemize}

However we can't apply the linear regression with odds since they are a non-negative measure (the won't never go below zero) \arrow we take instead the \textbf{log-odds/logit}
$$
...
\end{align*}
by doing so we get back the probability through the \textbf{linear regression applied within a sigmoid function} (the model is still linear, but in the log-odds scale):
$$
p_i = \sigma(\eta_i)=\sigma(\beta_0+\beta_1x_{i1}...+\beta_px_{ip})
$$
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{img/logisticreg/logistic_regression.png}
\end{figure}\hfill\\
One of the advantages of this type fo regression is that it never crosses the 0, allowing us to intepret its value as a probability, and instead at the extrems flatterns on [0,1]. Another good thing is that we are able to interpret the model:\\

\vspace{0.6cm}
\textbf{Coefficient's interpretation: categorical variables} \\
Our feature is something like this:
$$
...
\begin{align*}
& \log\left(\frac{p(x)}{1-p(x)}\right)=\beta_0+\beta_1x,\quad\log\left(\frac{p(x+1)}{1-p(x+1)}\right)=\beta_0+\beta_1(x+1) \\
& \\
...
The effect in the odds space is still constant $e^{\beta_1}$, but visualized as an exponential (multiplicative progression):
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{img/logisticreg/odds_slope.png}
\end{figure}

On probablities instead the ratio is not constant, but depends on the position of the logistic curve:
\begin{figure}[h]
    \centering
...
\begin{itemize}
    ...
    \item there is no \textbf{perfect separation} := there is no variable that divides perfecly the dataset
\end{itemize}
...
Since our predictors are binary $\{0,1\}\ni Y_i \sim Bernoulli(p_i)$ we have that $\prob(Y_i=y|X_i)=p_i^{y}(1-p_i)^{1-y}$ \arrow a good model should assign high probability to the outcomes that actually occurred and low to those not occurred \arrow a good model chooses $\beta$s so that the observed pattern are as plausible as possible.\\ 
To estimate the model we use MLE as always:
$$
L(\beta)= \prod_{i=1}^np_i^{y_i}(1-p_i)^{1-y_i} \rightarrow \ell(\beta)=\log L(\beta) = \sum_{i=1}^n y_i\log p_i + (1-y_i)\log (1-p_i)
$$
...
$$ 
where $full$ stand for our model and $null$ for the intercept-only \arrow the larger the better. The 2 is only to fit the statistic in a Chi-squared distribution, allowing to compute a p-vlaue and do our evaluation with that.\\

\vspace{0.5cm}
Another metric to evaluate our model is the \textbf{deviance} $-2\ell$ where $\ell$ is the log-likelihood, so the larger the better.\\
This statistic is useful when we want to compare two nested models (not really useful, just for completeness).

\vspace{0.5cm}
Another metric used to compare out model with the intercept-only one we can also use the \textbf{McFadden's pseudo-$R^2$} (not to be confused with the linear regression $R^2$)
$$
...
$$
if the values is large then the realtive p-value is low, giving strong evidence that the predictor is meaningful. As usual, if we build a confidence interval we get $\hat{\beta}_j \pm z_{1-\alpha/2}\ SE(\hat{\beta}_j)$ and if this interval contains zero, the coefficient might be 0/irrelevant.
However this is for the log-odds, if we want to do the analysis in the odds realm we have to make it exponential:
$$
\hat{\beta}_j \rightarrow e^{\hat{\beta}_j},\ \left[e^{\hat{\beta}_j^L}\ ,\ e^{\hat{\beta}_j^U} \right]
$$
if this interval in the odds-world contains 1 then the coefficient might be irrelevant.

% --------------------------------------------------------------------------------------------------------
\subsection{Classifier performance}
We can resume the outcome ouf our classfier in a \textbf{confusion matrix}: it compares the outcome of the model in predicting the datapoint classification with the true datapoint classification.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\linewidth]{img/logisticreg/classifier_performance.png}
\end{figure}\hfill\\
\textbf{Accuracy} := raw number of good evaluations made by the model, this metric is a little bit silly since it's influenced by level of unbalance of the dataset.

\vspace{0.2cm}
We could see that all thee values are dependet from the threshold chosen. Can we figure out a way to make the evaulation independent from this? We could think of trying out all possible thresholds, and choosing the one maximizing some metric.

\subsubsection{ROC curve \& AUC}
We evaluate the model by comparing its  \textit{sensitivity}  with its False Positive Ratio=1-\textit{specificity} for all the possible thresholds \textcolor{gray}{(each threshold gives you a diffrent ratio)}.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{img//logisticreg/ROC_curve.png}
\end{figure}\hfill\\

To obtain a numeric indication of this fit we compute the area if this curve, obtaining the \textbf{Area Under Curve (AUC)} metric: values near 0.5 indicate a predictor similar to the trivial one, instead for AUC near 1 we get the perfect predictor.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{img//logisticreg/auc.png}
\end{figure}
