...
The idea to solve the range issue is to change the undeling shape: we shift from the world of probabilities to the world of \textbf{odds} := how more likely is some event to happen with respect to not happen.
$$
 odds = \frac{p}{1-p}
$$
\begin{figure}[h]
...
The effect in the odds space is still constant $e^{\beta_1}$, but visualized as an exponential:
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{img/logisticreg/odds_slope.png}
\end{figure}

On probablities instead the ratio is not constant, but dependes on the position of the logistic curve:
\begin{figure}[h]
    \centering
...
This statistic is useful when we want to compare two nested models (not really useful).

\vspace{0.5cm}
Another metric used to compare out model with the intercept-only one we can also use the \textbf{McFdadden's pseudo-$R^2$} (not to be confused with the linear regression $R^2$)
$$
...
We compare the \textit{sensitivity} of the model with its FPR=1-\textit{specificity} for all the possible thresholds \textcolor{gray}{(each threshold gives you a diffrent ratio)}.
\begin{figure}[h]
    \centering
