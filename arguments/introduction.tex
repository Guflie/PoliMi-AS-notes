...
If we know the height of all students, we'd like to use the population mean $\mu$ as 
predictor, and by doing so we get as MSE $\mathbb{E}[(Y-\mu)^2]=Var(Y)=\sigma^2$.\\
\begin{center} \textcolor{red}{
    \textbf{The data variance is irreducible, we can't go beyond that.}   
} \end{center}
\hfill \\

However, it's impossible to have information about the whole population; so what we are
going to do is taking a small sample from it over and over, using the sample mean 
$\bar{Y}$ as predictor and getting $MSE_{avg}=\mathbb{E}[(\hat{Y}-Y)^2]$. By decomposing the 
average MSE we are able to understand which are our sources of error: \textit{bias} and \textit{variance}.
\begin{description}
...
So if we try to fit a model $f(X)$ with some training data, we end up with $\hat{f}(X)$, evaluating
it on some test observation $(x_0,y_0)$ (not in the training set) we have that
\begin{equation*}
    \expect[y_0-\hat{f}(x_0)] = Var[\hat{f}(x_0)] + Bias[\hat{f}(x_0)]^2 + Var(\varepsilon)
\end{equation*}
\begin{description}
    \item[Variance] $Var[\hat{f}(x_0)]$: captures how much our model changes based on the training dataset
    \item[Bias] $Bias[\hat{f}(x_0)]$: the inherent error we get from our models
    \item[Noise] $Var(\varepsilon)$: ambiguity due the data distribution   
\end{description}
\newpage

If we take a model too simple, we end up with something having high bias, but not really
susceptible to the specific training set (low variance) $\Rightarrow$ \textbf{Underfitting}.
\begin{figure}[h]
    \centering
...
sensitive to the data we sample for the training $\Rightarrow$ \textbf{Overfitting}.
\begin{figure}[h]
    \centering
...
two decreases, the other increases. This forces us to find some trade-off between these twos.
\begin{figure}[h]
    \centering
