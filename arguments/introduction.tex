...
methods. The main difference resides in the purpouse: \textit{ML models} are designed to 
make the most accurate prediction possible, \textit{statistical models} instead aim to 
make inference about the relatioship between the data.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{introduction/ml_statistic_intersection.png}
\end{figure}

In this course we are going to deal with \textit{multivarieated data}, meaning that each data
point contains several variables, defining their \textit{dimensions}. Representing this data 
...
As for ML, we have diffent classes of methods based on the available data and how we 
interpret them:
\begin{description}[itemsep=0.6cm]
    ...
    \begin{minipage}{0.9\textwidth}
        ...
        \end{wrapfigure}
        \hfill \\
        In this class we can idenitify a \textbf{target} variable $Y$ and a set of "input"
        ...
        Tipically in this set of method we try to make prediction about new target values or 
        finding those features that are more important to explain the targets.
    \end{minipage}
    ...
    \begin{minipage}{0.9\textwidth}
        ...
        \end{wrapfigure}
        \hfill \\
        In this case there aren't target varibales, but we all the varibales are features 
        ($f(X)\approx X$). \\ This set of models focuses on explloting structures and patterns
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{introduction/linear_quadratic_models_comparison.png}
...
\begin{equation*}
    MSE(\hat{y})=\frac{1}{n}\sum_{i=1}^n (\hat{y}_i-y_y)^2
\end{equation*}
...
\textcolor{red}{
    \textbf{The data variance is irriducible, we can't do better than that.}
}
\newline
...
\begin{description}
    ...
    high variance if our predictions are very sensitve on the sampling we draw from 
    the universe
\end{description}
\begin{figure}[h]
    \centering
...
\end{equation*}
\begin{description}
    \item[Varince]: captures how much our model changes based on the training dataset
    \item[Bias]: the inherith error we get from our models
...
suscpible to training set (low variance) $\Rightarrow$ \textbf{Underfitting}.
\begin{figure}[h]
    \centering
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\textwidth]{introduction/tradeoff_bias_variance.png}
\end{figure}
