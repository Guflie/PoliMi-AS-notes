...
the targets harder: let's assume that $x_1$ and $x_2$ are collinear
\begin{equation}
...
By switching to the Lagrangian relaxation:
\begin{equation*}
...
$\Rightarrow$ a candidate direction must be an \textbf{eigenvector of $\mathbf{\Sigma}$}.\\
...
\begin{equation*}
    Var_{\scriptscriptstyle TOT}= \sum_{j=1}^p Var(x_j) = \sum_{j=1}^p (\frac{1}{n}\sum_{i=1}^n
    x_{ij}^2)
\end{equation*}
...
have a clue about which dimensions go in the same direction, and from the give to the PC some kind
of interpetation
\begin{figure}[h]
    \centering
