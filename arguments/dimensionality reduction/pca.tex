...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/pca/pca_projection_example.png}
\end{figure}
...
level: it can be used also to set the data noise treshold in order to remove those components "making too much noise".
\begin{figure}[h]
    \centering
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{dr/pca/biplot_example.png}
\end{figure} \hfill \\
...
Up to now we've used the covariance matrix $\Sigma$ but this lead to sognificant results only if the data are on the same unit/scale, otherwise data with higher values will dominate the others.
This issue can be addresses in two ways:
\begin{itemize}
    \item using the correlation matrix
    \item standardize the data up to a 0-mean unit-variance $X_{ij}'=\cfrac{X_{ij}-\bar{X}}{\sigma_j}$
\end{itemize}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{img//dr//pca/pca_scaling.png}
\end{figure}
In general standardizing the data is the prefered solution, and it's done not only in PCA but also in all the methods that will follow.