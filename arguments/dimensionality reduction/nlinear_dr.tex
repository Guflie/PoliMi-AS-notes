...
The non-linear transformation is called \textbf{basis expansion} $\phi: \real^p \rightarrow 
...
\begin{figure}[h]
    ...
    right the distance is computed using local neighbourhood.}
    \label{fig:exlocdist}
\end{figure}\hfill \\
Note: This type of methods are really useful for data visualization in 2D/3D by performing some kind of embedding.\\
\hfill \\

As shown in the Fig \ref{fig:exlocdist} the problem with global similarity is that doesn't
strictly follows the data shape, while based on similarity between point we are able to adapt to it.
To do that, we don't need to preserve the distances with respect
to all the points, but just with neighbours. This allows to preserve the 
...
original space should remain close in the mapping, otherwise they aren't forced to 
preserve the "right far distance" (not global distance as focus, meaning that the extact global distance might not be preserved).

\vspace{0.3cm}

To do this we translate the distance into probability: if two points are close, they have higher
probability of being neighbours. To determine the neighbourhood concept, the t-SNE firstly determines the \textbf{similarity score} between points based on the distance between them using a \textit{probability distribution} to decrease smoothly the score based on the distance between points.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.55\textwidth]{dr/non_linear/tSNE-similarity.png}
\end{figure}

For each pair of points $i,j$ we define the \textbf{Gaussian neighbourhood 
distribution} (for example, other distributions can be used as well) as :
\begin{equation}
    p_{j|i}= \begin{cases}
...
\textcolor{gray}{(Think about it is as "the 
...
together, making the Normal distribution not suited for the job(it will end up considering all the 
points in the same cluster). The \textit{t-student} instead has heavier tails and lower hill, making the probability distribution more evenly spread\textcolor{gray}{(we repesent better our uncertanty on the inter-distance between points)}, leveraging the dimensionality reduction
...
By using different distributions for different dimensionality we end up with two different
\textbf{similarity matricies}, and our objective becomes making them look alike as much 
as possible, minimizing the mismatch between $p_{ij}$ and $q_{ij}$.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/similmat_comparison.png}
\end{figure}

So starting from some random initialization for the points placement, we use an iterative algorithm to move them in the low-D space.\\
...
the concept of neighbourhood, which is done by changing the preplexity ($perp. \propto \sigma^2$) $\rightarrow$ 
...
we build a graph in the high-D space linking the points and we try to obtain the same graph
on the low-D space in order to preserve the structure as much as possible.
\begin{figure}[h]
    \centering
...
overlapping circles are neighbours $\rightarrow$ the circle's radius hyperparameter choice is critical.\\
\begin{figure}[h]
    \centering
...
of fixed radius value, for each point we use the distance from its k-th neighbour as radius, and
...
\begin{description}
    ...
    \rightarrow$ the econder applies a non-linear function \ssigma := activation function on a 
    ...
    \rho(Vz+c)$; the structure is the same as encoder, just in the other way around (the hyperparameters used are \diff).
\end{description}
\begin{figure}[h]
    \centering
...
reconstruction phase, maybe even to remove only the information we want to loose $\rightarrow$
one of the main advantages of Autoencoders is that they allow us to decide what we lose.\\
\vspace{0.5cm}

\textbf{Training objective}: to tune the parameter sets $\theta,\phi$ in order to make the
reconstruction as loss-less as possible, we have to \textit{train the algoirthm}:
\begin{equation}
...
PCA, however still allow to tune the level information/varibaility we want to keep
$\rightarrow$ very useful to identify data anomaly: if a point is very far away from the original
space it will be projected far away, and also during the reconstrucion phase the decoder will try
...
\begin{description}
    \item[Kernel PCA]\hfill
        \begin{description}
            \item[\textcolor{green}{Pros}]: linearizes non-linear structure if a kernel can be found and
