...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.3\textwidth]{dr/non_linear/manifold_vis.png}
\end{figure}
PCa is a linear DR techinque, for real data we need techniques able to properly
model complex non-linear shapes.

%-----------------------------------------------------------------------------------------
\subsubsection{Kernel PCA methods}
This method famility follows the idea that \textit{data might be non-linear in $\real
^D$ but become "more linear" after a non-linear feature mapping $\phi()$}.
The non-linear transformation is called \textbf{bias expansion} and can be choosen
accordgnly to the analyst's assumption about the shape of the feature, $\phi: \real
^p \rightarrow \real^q$; usually we take $q>p$ and transform the sampled data into 
an higher-dimensional space.\\
\hfill \\

We define the basis-expanded matrix $\Phi=(\phi(x_1)\dots \phi(x_n))^T\rightarrow
\Sigma=\Phi^T\Phi$. However, for big-dimensional projections this approach could
be computationally expensive. In this case we can take advantage of the
\textbf{kernel trick}: we can compute $\Sigma_{i,j}=k(x_i,x_j)$ using a \textit{kernel 
function $k(,)$}, which measures the similarity between two projections and it's 
defined by $k(x_i,x_j)=\langle\phi(x_i),\phi(x_j)\rangle$ (scalar product).
\hfill \\

One of the advantages of this method is the ability of untangling circles.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/nldr_untangling_circles.png}
\end{figure}
\vspace{0.8cm}

The problem with Kernel PCA is the reconstructionof the original space: with linear
...
original one. In \textit{Kernel PCA} however $\phi$ is learned/induced by the data
and this could end up in having complex inverse transformations, if not even
non-invertible $\phi$. The only solution to this is to relay on learning the
pre-image of the trasmformation, or more commonly an approximated one.
% this images can be displayed side to side
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/inv_kernelPCA.png}    
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/kernel_reconst.png}    
\end{figure}
...
focusing on local neighbours (points near to each other should remain close after
the projection)}.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/global_vs_local_neigbhbours.png}
    \caption{On the left distance computed acconrdingly to global distance; on the
    right distance computed trought local neighbour.}
\end{figure}
This type of methods are really useful for data visualization.
\hfill \\

As reported in the figure, the problem with global similarity is that doesn't
striclty follows the data shape, while based on similarity between point we \dots
watch the recording. To do that, we don't need to preserve the distances with respect
...
To determine the negihbourhood concept, the t-SNE firstly determines the \textbf{
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/tSNE-similarity.png}
\end{figure}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
then to make the relationship simmetric $p_{ij} = \frac{p_{i|j}+p_{j|i}}{2}$.\\
To project as much as possibile these similarities on a low-dimensional spaces, instead
...
the probability distribution more evenly spread, leveragin the dimensionality reduction
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/umap_concept.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/umap_radius.png}
\end{figure}
...
using somethig similar to a dstance as weight of the edge.
\\
...
\begin{description}
    ...
    \item[Min distance]: tunes how much tyghtly data points are gonna be projected in the 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{dr/non_linear/autoencoders__sketch.png}
\end{figure}
...
\begin{itemize}
    ...
    execution time and accuracy (sometimes preserves also the gloabl organization); use 
    \textit{t-SNE} when you mainly concern on displaying local cluster separation.
    \item \textbf{Nonlinear feature engeineering (reusable mapping)}: \textit{autoencoders}
