...
$$
    \frac{\partial}{\partial\beta} \|y-X\beta\|_2^2 = -2X^T(y-X\beta)=0 \Rightarrow \textcolor{red}{\hat{\beta} = (X^TX)^{-1}X^Ty}
$$
...
\begin{itemize}
    ...
    \item with \textbf{variance} $\hat{\sigma}^2=\cfrac{RSS}{n-p}$ (although is the same formula of simple linear regression, now we have less degree of freedom)
\end{itemize}
...
\begin{figure}[h!]
    ...
    regression, therefore the extension to multiple predictor doesn't change this part.}
\end{figure}
...
\vspace{0.2cm}
Take predictor $X_j$ and build a model that predicts $X_j$ from all the other predictors and consider this $R_j$, we have that
\begin{align*}
...
Up to now we've made some assumptions about the error: $\mathbb{E}[\varepsilon|X]=0, Var(\varepsilon|
X)=\sigma^2, Cov(\varepsilon_i,\varepsilon_j|X)=0$ and on other aspects  of the model, but what if
these assumption doesn't hold?
...
If this assumption doesn't hold, we might have: variable bias, measurement errors in
...
\vspace{0.2cm}
\textcolor{green}{Good pattern}: random cloud centered around zero, without visible structure.\\
...
\begin{center}
    \textit{$H_0$: residuals are normal} \text{ vs }$\ H_1$\textit{: residuals are not normal}
\end{center}
